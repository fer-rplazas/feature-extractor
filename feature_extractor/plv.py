...
class PLVFeatureExtractor:
    def __init__(
        ...
        """
        Initializes the PLV Feature Extractor.
    ...
    def get_feats(
        ...
        fs: float = 2048.0,
    ) -> Float[np.ndarray, "n_epochs n_frames n_bands"]:
        """
        Computes the Phase Locking Value (PLV) between two signals across specified frequency bands.
        ...
        Returns:
            np.ndarray: PLV features with shape (n_epochs, n_frames, n_freq_bands).
        """
        ...
        for band_idx, (f_low, f_high) in enumerate(self.freq_bands):
            ...
            # Store the PLV values for the current frequency band
            feats[:, :, band_idx] = plv.astype(np.float32)
